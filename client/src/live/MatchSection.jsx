...
const MatchSection = ({ matches, title, emptyMessage, isLiveSection, activeLeague, leagues, getScoreDisplay, getStatusColor, formatStatus, isLiveMatch }) => (
      ...
      {matches.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {matches.map((match, index) => (
            <MatchCard key={match.matchId || match.fixtureId || index} match={match} index={index} getScoreDisplay={getScoreDisplay} getStatusColor={getStatusColor} formatStatus={formatStatus} isLiveMatch={isLiveMatch}  />
          ))}
        </div>
      )}
    </div>
